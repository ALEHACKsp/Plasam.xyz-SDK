...
namespace ImGui
{
    ...
    IMGUI_API void          PushStyle(const ImGuiStyle& style);
    IMGUI_API void          PopStyle();
    ...
    IMGUI_API ImVec2        CalcItemRectClosestPoint(const ImVec2& pos, bool on_edge = false, float outward = +0.0f);   // utility to GetOffset the closest point the last item bounding rectangle edge. useful to visually link items
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_AlwaysUseWindowPadding = 1 << 16,  // Ensure child windows without border uses style.WindowPadding (ignored by default for non-bordered child windows, because more convenient)
                                                        ...
                                                        ImGuiWindowFlags_GroupBox = 1 << 28   // Don't use! For internal use by BeginGroupBox()
};
...
struct ImGuiIO
{
    ...
    bool        KeysDown[512];              // Keyboard m_iKeyMap that are pressed (in whatever storage order you naturally have access to keyboard data)
...
struct ImGuiStorage
{
    ...
    // - Get***() functions GetOffset pair, never add/allocate. Pairs are sorted so a query is O(log N)
    // - Set***() functions GetOffset pair, insertion on demand if missing.
...
// ImColor() helper to implicity converts colors to either ImU32 (packed 4x1 uint8_t) or ImVec4 (4x1 float)
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont*           AddFontFromSystem(const char* name, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL);
    ...
    IMGUI_API void              GetTexDataAsAlpha8(unsigned char** out_pixels, int* out_width, int* out_height, int* out_bytes_per_pixel = NULL);  // 1 uint8_t per-pixel
